...
class PythonEnvironment {
  ...
  createEnvironment() {
    if (!fs.existsSync(this.venvPath)) {
      ...
      try {
        ...
        return {status: false, message: e.message+"\nmake sure you installed python3.7 with venv support"};
      }
    } else {
  ...
  runCommand(command) {
    ...
    const activatePath =this.isWindows()? path.join(this.venvPath, activateScript): `. ./`+path.join(this.venvPath, activateScript);
    const activateCommand = this.isWindows() ? activatePath : `${activatePath}`;
